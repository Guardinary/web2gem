...
import { buildImageResponsesOutput, buildResponsesOutput, finalizeOpenAICompletionResult, openAIResponsesUsage } from "./format";
...
export async function handleResponses(req: Record<string, unknown> | undefined, cfg: RuntimeConfig, provider: CompletionProvider) {
  ...
  if (imageMode.enabled) return handleImageGenerationResponses(req, cfg, provider, imageMode.forced);
...
async function handleImageGenerationResponses(req: Record<string, unknown>, cfg: RuntimeConfig, provider: CompletionProvider, forced: boolean): Promise<Response> {
  ...
  try {
    ...
    if (logRequests) logStage(cfg, "openai_responses_image_generate", { ms: elapsedMs(generationStart), status: "error", model: rm.name });
    log(cfg, `openai responses image generate failed model=${rm.name} code=${upstreamErrorCode(e) || "upstream_error"} error=${errorLogSummary(e)}`);
    return openAIUpstreamErrorResponse(e);
  }
  ...
  const rid = `resp_${randHex(16)}`;
  const mid = `msg_${randHex(12)}`;
  const output = buildImageResponsesOutput(rich.text, rich.images, mid, () => `ig_${randHex(12)}`);
  return jsonResponse({
    id: rid,
    object: "response",
    created_at: nowSec(),
