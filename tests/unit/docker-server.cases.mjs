...
import { createD1HttpBinding, resolveD1HttpConfig } from "../../scripts/d1-http-binding.mjs";
import { createDockerServer, requestHeaders, requestUrl, resolveDockerEnv } from "../../scripts/docker-server.mjs";
...
export const cases = [
  ...
  ["injects Docker D1 binding only for complete HTTP config", async () => {
    assert.equal(resolveD1HttpConfig({}), null);
    try {
      resolveDockerEnv({ D1_ACCOUNT_ID: "account", D1_API_TOKEN: "token-secret-fragment" });
  ...
  ["maps D1 HTTP first all and run without leaking params or tokens in errors", async () => {
    const requests = [];
    const responses = [
      { success: true, result: [{ results: [{ value: "first-row" }], meta: { changes: 0 } }] },
      { success: true, result: [{ results: [{ id: 1 }, { id: 2 }], meta: { changes: 0 } }] },
    ...
    assert.equal(await binding.prepare("SELECT ? AS value").bind("__Secure-1PSID=secret-cookie").first("value"), "first-row");
    assert.deepEqual((await binding.prepare("SELECT * FROM t").all()).results, [{ id: 1 }, { id: 2 }]);
    assert.deepEqual(await binding.prepare("UPDATE t SET a = ?").bind("session-token-secret").run(), {
      success: true,
      meta: { changes: 3 },
    });
    assert.match(requests[0].url, /\/accounts\/account\/d1\/database\/database\/query$/);
    assert.equal(requests[0].init.headers.authorization, "Bearer d1-token-secret");
    ...
    const failing = createD1HttpBinding({
      ...
      async fetch() {
        return new Response(JSON.stringify({
          success: false,
          errors: [{ code: "7500", message: "bad __Secure-1PSID=secret-cookie session-token-secret d1-token-secret" }],
        }), { status: 200 });
      },
    });
    await assert.rejects(
      () => failing.prepare("SELECT ?").bind("__Secure-1PSID=secret-cookie", "session-token-secret").all(),
      /D1 HTTP query failed code=7500/,
    );
    try {
      await failing.prepare("SELECT ?").bind("__Secure-1PSID=secret-cookie", "session-token-secret").all();
    } catch (err) {
      const message = String(err.message || err);
      assert.doesNotMatch(message, /secret-cookie|session-token-secret|d1-token-secret/);
    }
    ...
    const fetchThrows = createD1HttpBinding({
      ...
      async fetch() {
        throw new Error("network body __Secure-1PSID=secret-cookie session-token-secret d1-token-secret");
      },
    });
    try {
      await fetchThrows.prepare("SELECT ?").bind("__Secure-1PSID=secret-cookie", "session-token-secret").all();
      throw new Error("expected D1 fetch failure to throw");
    } catch (err) {
      assert.match(err.message, /D1 HTTP query failed before response/);
      assert.doesNotMatch(err.message, /secret-cookie|session-token-secret|d1-token-secret/);
    }
  }],
];
