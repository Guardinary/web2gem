...
| Automatic account failover   | Try up to three distinct eligible accounts for account-scoped failures, excluding failed, disabled, cooling-down, or otherwise unavailable accounts. |
...
| `RETRY_ATTEMPTS`                | `3`                         | Same-upstream retry attempts for anonymous requests. Managed-account failures instead use the bounded three-account failover path.                                                                              |
