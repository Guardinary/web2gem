...
describe("google http", () => {
	...
	test("adds DSML tool instructions for Google function declarations", async () => {
		const prompts = [];
		const provider = {
	...
	test("adds DSML tool instructions for wrapped Google function declarations", async () => {
		const prompts = [];
		const provider = {
			async generateText(input) {
				prompts.push(input.prompt);
	...
	test("keeps Google image refs before context refs while appending generic refs", async () => {
	...
	test("filters Google OpenAI-style function tools by config", async () => {
		const cfg = {
			current_input_file_enabled: false,
			current_input_file_min_bytes: 1000000,
			current_input_file_name: "message.txt",
		...
		const filtered = filterGoogleToolsByConfig(req.tools, req);
	...
	test("filters Google schema shorthand tools by config", async () => {
		const cfg = {
			current_input_file_enabled: false,
			current_input_file_min_bytes: 1000000,
			current_input_file_name: "message.txt",
	...
	test("filters Google functionDeclarations arrays by config", async () => {
		const cfg = {
			current_input_file_enabled: false,
			current_input_file_min_bytes: 1000000,
			current_input_file_name: "message.txt",
	...
	test("filters Google functions arrays with parametersJsonSchema", async () => {
		const cfg = {
			current_input_file_enabled: false,
			current_input_file_min_bytes: 1000000,
			current_input_file_name: "message.txt",
	...
	test("validates Google tool-choice mode and allowed-name aliases", async () => {
		const tools = [
			{
				functionDeclarations: [
	...
	test("omits Google tools when function calling mode is NONE", async () => {
		const filtered = filterGoogleToolsByConfig(
			[
				{
					functionDeclarations: [
						{ name: "Read", parameters: { type: "object" } },
	...
	test("rejects Google plain answer when ANY requires a tool call", async () => {
		const tools = [
			{
				functionDeclarations: [
					{ name: "Read", parameters: { type: "object" } },
	...
	test("emits Google tool stream policy violation instead of candidate text", async () => {
		const tools = [
			{
				functionDeclarations: [
					{ name: "Read", parameters: { type: "object" } },
		...
		const events = [];
		for await (const event of streamGoogleToolCompletionEvents(
	...
	test("emits Google tool stream protocol error before any output", async () => {
		const events = [];
		for await (const event of streamGoogleToolCompletionEvents(
			fakeProvider({
				streamText() {
	...
	test("emits Google tool stream warning after partial plain output", async () => {
		const events = [];
		for await (const event of streamGoogleToolCompletionEvents(
			fakeProvider({
				streamText() {
	...
	test("emits Google tool stream warning before parsed function calls", async () => {
		const tools = [
			{
				functionDeclarations: [
					{ name: "Read", parameters: { type: "object" } },
	...
	test("omits tool instructions for plain Google requests", async () => {
		const prompts = [];
		const provider = {
			async generateText(input) {
				prompts.push(input.prompt);
	...
	test("converts Google system image function call and function response parts", async () => {
		const prompts = [];
		const provider = {
			async generateText(input) {
				prompts.push(input.prompt);
	...
	test("streams Google plain deltas and usage done payload", async () => {
		const writes = [];
	...
	test("streams Google upstream error before any output", async () => {
		const writes = [];
		const logs = [];
		await withConsoleLog(
			(line) => logs.push(String(line)),
	...
	test("streams Google tool function calls and done usage", async () => {
		const writes = [];
		const tools = [
			{
				functionDeclarations: [
