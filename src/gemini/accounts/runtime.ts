...
export function createGeminiAccountRuntimeFromEnv(
  ...
  return new GeminiAccountRuntime(new AccountPoolService(new D1GeminiAccountStore(db), {
    ...options,
    rotateCookie,
  }));
}

export function d1BindingFromEnv(env: WorkerEnv | null | undefined): D1DatabaseLike | null {
...
async function defaultRotateCookie(input: { config: RuntimeConfig; account: GeminiAccountSecretRow }): Promise<GeminiAccountRotateResponse> {
  return httpFetch("https://accounts.google.com/RotateCookies", {
    ...
    timeoutMs: Math.min(Math.max(Number(input.config.request_timeout_sec) || 30, 1) * 1000, 30000),
