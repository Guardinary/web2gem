const REQUIRED_D1_HTTP_ENV = ["D1_ACCOUNT_ID", "D1_DATABASE_ID", "D1_API_TOKEN"];
...
export function resolveD1HttpConfig(env = process.env) {
  ...
  const present = [values.accountId, values.databaseId, values.apiToken].filter(Boolean).length;
  if (present === 0) return null;
  if (present !== 3) {
    throw new D1HttpBindingError(`partial D1 HTTP configuration: ${REQUIRED_D1_HTTP_ENV.filter((name) => !clean(env[name])).join(", ")} missing`, {
...
export function createD1HttpBinding(config, options = {}) {
  ...
  if (typeof fetchImpl !== "function") {
    throw new D1HttpBindingError("D1 HTTP binding requires fetch", { code: "d1_http_fetch_missing" });
  }
...
class D1HttpPreparedStatement {
  ...
  async query() {
    ...
    try {
      ...
      throw new D1HttpBindingError("D1 HTTP query failed before response", { code: "d1_http_fetch_error", status: 502 });
    }
    if (!response || !response.ok) {
      throw new D1HttpBindingError(`D1 HTTP query failed status=${Number(response?.status) || 0}`, {
        code: "d1_http_status",
    ...
    try {
      ...
      throw new D1HttpBindingError("D1 HTTP query returned invalid JSON", { code: "d1_http_invalid_json" });
    }
    const result = normalizeD1HttpPayload(payload);
    if (!payload || payload.success === false || result.success === false) {
      throw new D1HttpBindingError(safeCloudflareErrorMessage(payload), { code: "d1_http_api_error", status: 502 });
    }
    return result;
  }
}
