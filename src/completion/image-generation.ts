import { base64ToBytes, detectUploadMimeFromBytes, firstNonEmptyString, imageFilenameFromMime, imageFilenameFromObject, normalizeMimeType, normalizeUploadFileInput, parseImageUrl, sanitizeUploadFilename, uploadFilenameFromObject, uploadMimeFromObject } from "../attachments/media";
import { MAX_ATTACHMENTS_PER_REQUEST } from "../attachments/plan";
import type { AttachmentCandidate, AttachmentFileRef, AttachmentPlan } from "../attachments/types";
...
import type { AttachmentResolutionResult, FileRef, LooseRequest } from "./types";
...
const FORCED_IMAGE_GENERATION_INSTRUCTION = "Image generation was explicitly requested. Return at least one generated image; a response without a generated image is a failure.";
...
export async function prepareOpenAIImageGenerationCompletion(
  ...
  return prepareImageGenerationFromState(cfg, provider, req.model, state, forced);
}
...
export async function prepareOpenAIImageGenerationFromUserInput(
  ...
  return prepareImageGenerationFromState(cfg, provider, input.model, state, forced);
}
...
async function prepareImageGenerationFromState(
  ...
  if (!cfg.cookie) {
    return {
      error: {
        message: "image generation requires configured GEMINI_COOKIE",
        status: 401,
  ...
  if (rm.name === undefined) {
    log(cfg, `openai image generation model rejected model=${String(model ?? "(default)")}`);
    return { error: { message: rm.error, status: 400, code: "model_not_found" } };
  }

  if (state.error) return { error: state.error };

  const userPrompt = state.textParts.map((part) => part.trim()).filter(Boolean).join("\n").trim();
  if (!userPrompt) {
    return { error: { message: "image generation requires non-empty user prompt text", status: 400, code: "image_generation_empty_prompt" } };
  }

  const prompt = [userPrompt, IMAGE_GENERATION_INSTRUCTION, forced ? FORCED_IMAGE_GENERATION_INSTRUCTION : ""].filter(Boolean).join("\n\n");
...
function extractResponsesUserInput(state: ExtractionState, input: unknown): void {
...
function extractResponseItem(state: ExtractionState, item: unknown): void {
  ...
  if (typ === "output_text" || typ === "summary_text" || typ === "reasoning" || typ === "thinking" || typ === "function_call" || typ === "function_call_output") return;
...
function extractLatestChatUserInput(state: ExtractionState, messages: unknown): void {
...
function extractContentPart(state: ExtractionState, part: unknown): void {
  ...
  if (typ === "output_text" || typ === "summary_text" || typ === "reasoning" || typ === "thinking") return;
...
function appendImagePart(state: ExtractionState, part: UnknownRecord): void {
  if (remoteUrlFromPart(part)) {
    state.error = unsupportedImageInput("remote image/file URLs are not supported in image generation mode");
    return;
  }
...
function appendImageBytes(state: ExtractionState, image: ImageGenerationByteInput): void {
  const detected = detectUploadMimeFromBytes(image.bytes);
  if (!normalizeMimeType(detected).startsWith("image/")) {
    state.error = unsupportedImageInput("image input bytes are not a supported image");
    return;
  }
  ...
  const filename = firstNonEmptyString(sanitizeUploadFilename(image.filename), imageFilenameFromMime(mime, state.nextID));
...
function appendFilePart(state: ExtractionState, part: UnknownRecord): void {
  ...
  if (!input || input.b64 == null) {
    state.error = unsupportedImageInput("image generation file input must be an inline payload or existing file reference");
    return;
  }
  ...
  if (!normalizeMimeType(detected).startsWith("image/")) {
    state.error = unsupportedImageInput("image generation file input bytes are not a supported image");
    return;
  }
  const mime = firstNonEmptyString(detected, input.mime, uploadMimeFromObject(part), "image/png");
  const candidate: AttachmentCandidate = {
    id: `att_${state.nextID}`,
    kind: "image",
    role: "request",
  ...
  const filename = firstNonEmptyString(sanitizeUploadFilename(input.filename), uploadFilenameFromObject(part), imageFilenameFromMime(mime, state.nextID));
  if (filename) candidate.filename = filename;
  if (mime) candidate.mime = mime;
  addCandidateSlot(state, candidate);
}

function imageCandidateFromPart(part: UnknownRecord, nextID: number): { candidate: AttachmentCandidate } | { error: ImageGenerationPrepareError } {
  const parsed = imagePayloadFromPart(part);
  if (!parsed) return { error: unsupportedImageInput("image input must be an inline image payload or existing file reference") };
  let bytes: Uint8Array;
  try {
  ...
  const detected = detectUploadMimeFromBytes(bytes);
  if (!normalizeMimeType(detected).startsWith("image/")) {
    return { error: unsupportedImageInput("image input bytes are not a supported image") };
  }
  const mime = firstNonEmptyString(detected, parsed.mime, uploadMimeFromObject(part), "image/png");
  const candidate: AttachmentCandidate = {
    id: `att_${nextID}`,
    kind: "image",
    role: "request",
  ...
  const filename = firstNonEmptyString(imageFilenameFromObject(part), imageFilenameFromMime(mime, nextID));
  if (filename) candidate.filename = filename;
  if (mime) candidate.mime = mime;
  return { candidate };
}

function imagePayloadFromPart(part: UnknownRecord): { b64: unknown; mime: string } | null {
  const source = isRecord(part.source) ? part.source : null;
  if (source && source.data != null) {
    return { b64: source.data, mime: firstNonEmptyString(uploadMimeFromObject(part), source.media_type, source.mime_type, source.mimeType, "image/png") };
  }
...
function addCandidateSlot(state: ExtractionState, candidate: AttachmentCandidate): void {
  if (state.candidates.length >= MAX_ATTACHMENTS_PER_REQUEST) {
    state.error = { message: `image generation supports at most ${MAX_ATTACHMENTS_PER_REQUEST} user attachments`, status: 400, code: "image_input_unsupported" };
    return;
  }
...
async function resolveImageGenerationFileRefs(provider: CompletionProvider, state: ExtractionState): Promise<{ fileRefs: FileRef[] | null } | { error: ImageGenerationPrepareError }> {
  if (!state.candidates.length && !state.slots.some((slot) => slot.type === "existing")) return { fileRefs: null };
  const plan: AttachmentPlan = {
    candidates: state.candidates,
    existingFileRefs: state.slots.filter((slot): slot is { type: "existing"; ref: AttachmentFileRef } => slot.type === "existing").map((slot) => slot.ref),
    dropped: [],
  ...
  for (const slot of state.slots) {
    ...
    if (!ref) {
      return { error: { message: "failed to upload image generation input", status: 502, code: "image_input_upload_failed" } };
    }
    out.push(ref);
  }
...
function existingRefFromPart(part: UnknownRecord): AttachmentFileRef | null {
  const id = part.file_id ?? part.fileId ?? part.file_ref ?? part.fileRef ?? part.ref;
  if (id == null) {
    const file = isRecord(part.file) ? part.file : null;
    const nested = file ? (file.file_id ?? file.fileId ?? file.file_ref ?? file.fileRef ?? file.ref ?? file.id) : null;
    if (nested == null) return null;
    const name = firstNonEmptyString(uploadFilenameFromObject(file), uploadFilenameFromObject(part));
...
function remoteUrlFromPart(part: UnknownRecord): string {
  ...
  const source = isRecord(part.source) ? part.source : null;
  const sourceUrl = source ? firstNonEmptyString(source.url, source.file_url, source.fileUrl, source.file_uri, source.fileUri) : "";
  ...
  const file = isRecord(part.file) ? part.file : null;
  const fileUrl = file ? firstNonEmptyString(file.url, file.file_url, file.fileUrl) : "";
  if (isRemoteUrl(fileUrl)) return fileUrl;
  const fileData = isRecord(part.fileData) ? part.fileData : (isRecord(part.file_data) ? part.file_data : null);
  const nestedUrl = fileData ? firstNonEmptyString(fileData.url, fileData.file_uri, fileData.fileUri) : "";
