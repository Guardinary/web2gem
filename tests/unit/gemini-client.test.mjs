...
function fakeRuntime(leases) {
	return {
		...
		async routeCandidatesForModel(model) {
			return model.family ? [basicRouteForFamily(model.family)] : [];
		},
	};
}
...
describe("gemini client", () => {
	...
	test("strips generated code artifacts from Gemini text", async () => {
		const source = [
			"keep",
	...
	test("extracts longest response text from WRB lines", async () => {
	...
	test("extracts rich web image metadata and card text", async () => {
	...
	test("strips generated-image placeholder text while keeping rich images", async () => {
	...
	test("prefers completed or richer repeated candidate states", async () => {
		...
		const completedParts = extractResponseParts(completedFirst);
		...
		const keepCompletedParts = extractResponseParts(keepCompleted);
		...
		const richerParts = extractResponseParts(
	...
	test("handles malformed rich envelopes and invalid framed chunks without throwing", async () => {
		...
		assert.equal(extractResponseParts(")]}'\n\n0\n[]").candidateCount, 0);
		assert.equal(extractResponseParts(")]}'\n\n999\n[]").candidateCount, 0);
		assert.equal(extractResponseParts(")]}'\n\n5\nnot-json").candidateCount, 0);
		assert.equal(extractResponseParts(")]}'\n\n1x\n[]").candidateCount, 0);
	});
	test("extracts image-to-image generated image path and does not merge alternatives", async () => {
		const first = [];
	...
	test("does not attach alternative candidate text to selected image-only candidate", async () => {
	...
	test("keeps default first-candidate selection even when alternatives contain images", async () => {
	...
	test("extracts rich generated images from length-prefixed frames", async () => {
	...
	test("maps numeric Gemini fatal part codes from inner payloads and envelopes", async () => {
		assert.equal(extractResponseParts(fatalWrbLine(1013)).fatalCode, "1013");
		assert.equal(
			extractResponseParts(fatalWrbLine(1052, "envelope")).fatalCode,
			"1052",
		);
		assert.equal(extractResponseFatalCode(fatalWrbLine(1037)), "1037");
		assert.match(
			richResponseShapeSummary(fatalWrbLine(1060)),
			/fatalCode=1060/,
		);
	});
	test("surfaces fatal semantics before text and stream empty-response handling", async () => {
		const cfg = baseGeminiClientConfig();
		await withFetch(
			async () => new Response(fatalWrbLine(1037), { status: 200 }),
			async () => {
				try {
					await generate(cfg, "limited", 1, false, null);
				} catch (err) {
	...
	test("dedupes repeated rich generated image frames", async () => {
		const raw = [
	...
	test("preserves generated image URL candidates and browser headers", async () => {
	...
	test("fetches direct gg-dl generated image URLs before trying size suffix fallback", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "boq_test",
			cookie: "",
	...
	test("bounds individual and aggregate generated image hydration", async () => {
		...
		await withFetch(
			...
			async () => {
				const oversized = await hydrateGeneratedImages(
					cfg,
					cfg,
					[
						{
		...
		await withFetch(
			...
			async () => {
				const images = await hydrateGeneratedImages(
					cfg,
					cfg,
					[
	...
	test("fetches s1024 generated image fallback URLs and detects jpeg bytes", async () => {
		const cfg = baseGeminiClientConfig();
	...
	test("detects gif and webp generated image bytes", async () => {
		const cfg = baseGeminiClientConfig();
		const imageCases = [
			{
	...
	test("keeps web-only rich images without fetching image bytes", async () => {
		const cfg = baseGeminiClientConfig();
		const calls = [];
		await withFetch(
			async (url) => {
			...
			async () => {
				const rich = await generateRich(cfg, "show web image", 1, false, null);
				assert.equal(rich.images.length, 1);
	...
	test("maps rich fatal and empty upstream responses to image-specific errors", async () => {
		const cfg = baseGeminiClientConfig();
		await withFetch(
	...
	test("encodes fetched generated image bytes without TypedArray base64 helpers", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "boq_test",
			cookie: "",
	...
	test("rejects non-image generated image bodies even with image content-type", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "boq_test",
			cookie: "",
	...
	test("summarizes WRB parse issue branches without throwing", async () => {
	...
	test("streams only new text deltas from repeated WRB lines", async () => {
	...
	test("streams long cumulative WRB text without losing append state", async () => {
		const extractor = createStreamTextExtractor();
		let cumulative = "";
		let emitted = "";
		for (let i = 0; i < 512; i++) {
	...
	test("streams visible deltas after artifact-bearing cumulative chunks", async () => {
		const extractor = createStreamTextExtractor();
		const artifact = [
			"answer",
			"```python?code_reference&code_event_index=1",
	...
	test("builds Gemini payload with model number and extended thinking", async () => {
	...
	test("parses and merges cookie headers with quoted values", async () => {
	...
	test("keeps bounded account-scoped metadata hot across alternating accounts", async () => {
		const cache = createMemoryCache();
		const metadata = createOriginScopedStringCache({
	...
	test("caches Gemini build labels in the Workers cache API", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
		...
		await withCaches(cache, async () => {
			...
			await setCachedGeminiBuildLabel(cfg, "cached-bl");
			assert.equal(await getCachedGeminiBuildLabel(cfg), "cached-bl");
			assert.equal(cache.stats.match, 1);
	...
	test("persists Gemini build labels with waitUntil when available", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "configured-bl",
			log_requests: false,
		...
		await withCaches(cache, async () => {
			...
			assert.equal(await getCachedGeminiBuildLabel(cfg), "waituntil-bl");
			assert.equal(cache.stats.match, 0);
	...
	test("drops stale cached Gemini build labels", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "configured-bl",
			log_requests: false,
	...
	test("refreshes Gemini build labels once for concurrent callers", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			cookie: "SID=ok",
			upstream_socket: false,
		...
		await withCaches(cache, async () => {
			await withFetch(
				...
				async () => {
					const first = getFreshGeminiBuildLabel(cfg);
					const second = getFreshGeminiBuildLabel(cfg);
					release();
					...
					assert.equal(calls, 1);
					assert.equal(await getCachedGeminiBuildLabel(cfg), "fresh-bl");
				},
			);
		});
	});
	...
	test("redacts cookies from invalid cookie diagnostics", async () => {
	...
	test("observes managed account cookies from app and generation responses", async () => {
		resetGeminiUploadCachesForTest();
		const observed = [];
		const cfg = baseGeminiClientConfig({
	...
	test("streams fallback text when Gemini response has no body", async () => {
		resetActiveGeminiCookieForTest();
		resetGeminiUploadCachesForTest();
		const cfg = {
			gemini_origin: "https://gemini.example",
	...
	test("adapts resolved models through the Gemini completion provider", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "boq_test",
			cookie: "",
		...
		const rm = resolveModel("gemini-3.1-pro", "gemini-3.5-flash");
		await withFetch(
			async (url, init) => {
				assert.match(String(url), /StreamGenerate/);
	...
	test("routes an unknown dynamic model through one exact lease tuple", async () => {
		const cfg = baseGeminiClientConfig({
	...
	test("rejects dynamic generation when a runtime lease omits the exact route", async () => {
		const cfg = baseGeminiClientConfig();
		const lease = fakeLease(
	...
	test("logs Gemini routing fields through the completion provider", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "boq_test",
			cookie: "",
		...
		const rm = resolveModel("gemini-3.1-pro-extended", "gemini-3.5-flash");
		const logs = [];
		await withConsoleLog(
			(line) => logs.push(String(line)),
			async () => {
	...
	test("streams text through the Gemini completion provider and rejects unresolved models", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "boq_test",
			cookie: "",
	...
	test("forwards image resolution and text uploads through the Gemini completion provider", async () => {
		const cfg = {
			gemini_origin: "https://gemini.example",
			gemini_bl: "boq_test",
			cookie: "",
		...
		assert.deepEqual(
			await provider.resolveAttachments(createAttachmentPlan()),
			{
				fileRefs: null,
				imageFileRefs: null,
	...
	test("reuses one account lease across provider upload and text generation", async () => {
		...
		const attachments = await provider.resolveAttachments(
			createAttachmentPlan({
				files: [{ b64: "aGVsbG8=", mime: "text/plain", filename: "file.txt" }],
			}),
		);
		const text = await provider.generateText({
			prompt: "provider prompt",
	...
	test("marks upload failures and releases the selected account lease", async () => {
	...
	test("delegates exact provider arguments and filters empty stream deltas", async () => {
		const calls = [];
		const logs = [];
		const cfg = baseGeminiClientConfig({
			log_requests: true,
		...
		assert.deepEqual(calls[4].args, [cfg, createAttachmentPlan()]);
	...
	test("routes prompt threshold edges between anonymous and account generation", async () => {
		const anonymousRuntime = fakeRuntime([null]);
		const anonymousProvider = createGeminiCompletionProvider(
		...
		const accountProvider = createGeminiCompletionProvider(
			baseGeminiClientConfig({ current_input_file_min_bytes: 4 }),
			{
				accountRuntime,
				client: {
	...
	test("applies the selected account capacity to provider model headers", async () => {
		const model = resolveModel("gemini-3.1-pro", "gemini-3.5-flash");
		const modelId = basicRouteForFamily("pro").providerModelId;
	...
	test("falls back from anonymous errors to one account lease", async () => {
	...
	test("returns account errors when anonymous fallback generation also fails", async () => {
		const anonymousError = new Error("anonymous failed");
		const accountError = new Error("account failed");
		const lease = fakeLease(accountCfg("fallback-fail", "fallback-fail-hash"));
		const provider = createGeminiCompletionProvider(baseGeminiClientConfig(), {
			accountRuntime: fakeRuntime([lease]),
			client: {
				async generate(activeCfg) {
	...
	test("preserves anonymous errors when fallback has no account", async () => {
	...
	test("does not acquire fallback accounts for anonymous aborts", async () => {
	...
	test("falls back account streams only before anonymous output", async () => {
	...
	test("preserves anonymous stream errors when fallback has no account", async () => {
	...
	test("marks account stream failures after anonymous fallback", async () => {
	...
	test("uses the default unresolved-model error", async () => {
		const provider = createGeminiCompletionProvider(baseGeminiClientConfig());
		await assert.rejects(
	...
	test("keeps anonymous Flash standard and extended generation header-free", async () => {
		...
		assert.equal(
			await provider.generateText({
				prompt: "prompt",
				rm: resolveModel("gemini-3.5-flash-extended", "gemini-3.5-flash"),
				fileRefs: null,
			}),
			"anonymous",
		);
	...
	test("returns typed authenticated-session errors when no pool is configured", async () => {
		const provider = createGeminiCompletionProvider(baseGeminiClientConfig());
		await assert.rejects(async () => {
			try {
				await provider.generateText({
	...
	test("returns sanitized no-account errors for account-required models", async () => {
		let generated = false;
		const runtime = fakeRuntime([null]);
		const provider = createGeminiCompletionProvider(baseGeminiClientConfig(), {
			accountRuntime: runtime,
	...
	test("finalizes account streams only after iterator completion", async () => {
	...
	test("marks account stream failures and releases the selected lease", async () => {
	...
	test("keeps successful results when background outcome persistence fails", async () => {
		const background = [];
	...
	test("schedules stale account session maintenance after successful output", async () => {
		const background = [];
	...
	test("isolates opportunistic session maintenance failures from successful output", async () => {
		const background = [];
		const logs = [];
	...
	test("keeps account results when waitUntil registration throws", async () => {
	...
	test("preserves the original upstream error when outcome persistence fails", async () => {
	...
	test("fails over account-scoped text errors to an excluded alternate account", async () => {
	...
	test("rejects a runtime that returns an already-attempted account", async () => {
	...
	test("stops account failover at the configured distinct-account budget", async () => {
		const runtime = fakeRuntime([
	...
	test("rejects account acquisition after the provider is disposed", async () => {
		const provider = createGeminiCompletionProvider(baseGeminiClientConfig(), {
			accountRuntime: fakeRuntime([]),
		});
	...
	test("switches accounts for model inconsistency without switching for static header errors", async () => {
		...
		const headerProvider = createGeminiCompletionProvider(
			baseGeminiClientConfig(),
			{
	...
	test("returns the third account error after exhausting the request budget", async () => {
	...
	test("keeps model errors and aborts on the selected account", async () => {
		for (const error of [
	...
	test("retries one authentication failure on the refreshed account", async () => {
	...
	test("fails over streams only before the first visible account delta", async () => {
		...
		const beforeProvider = createGeminiCompletionProvider(
			baseGeminiClientConfig(),
			{
		...
		const afterProvider = createGeminiCompletionProvider(
			baseGeminiClientConfig(),
			{
	...
	test("replays generated upload recipes and remaps refs on account failover", async () => {
	...
	test("fails over rich generation and anonymous-account chains", async () => {
		const richRuntime = fakeRuntime([
			fakeLease(accountCfg("rich-a", "hash-a")),
			fakeLease(accountCfg("rich-b", "hash-b")),
		]);
		const richProvider = createGeminiCompletionProvider(
			baseGeminiClientConfig(),
			{
		...
		const seen = [];
		const chainProvider = createGeminiCompletionProvider(
			baseGeminiClientConfig(),
			{
	...
	test("keeps failover working when an intermediate outcome write rejects", async () => {
	...
	test("continues failover after refresh and synchronous outcome failures", async () => {
	...
	test("rejects selector reuse of an already attempted account", async () => {
	...
	test("returns a replay error when replacement uploads lose refs", async () => {
		const runtime = fakeRuntime([
	...
	test("keeps request-scoped and post-output account stream errors on one lease", async () => {
		...
		const fallbackProvider = createGeminiCompletionProvider(
			baseGeminiClientConfig(),
			{
