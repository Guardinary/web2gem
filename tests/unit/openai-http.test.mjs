...
describe("openai http", () => {
	...
	test("covers image generation preparation validation branches", async () => {
	...
	test("extracts image generation prompt and reference variants", async () => {
	...
	test("returns image provider unsupported and upstream errors consistently", async () => {
	...
	test("parses additional OpenAI Images endpoint option edges", async () => {
		let generated = 0;
		const provider = fakeProvider({
			async generateRich() {
				generated += 1;
	...
	test("maps OpenAI context upload failures during prepare", async () => {
		let generated = false;
		const uploadErr = new Error("upload refused");
		uploadErr.status = 504;
	...
	test("rejects oversized inline context before resolving request-local attachments", async () => {
		let generated = false;
		const resp = await handleChat(
			{
				model: "gemini-3.5-flash",
	...
	test("adds DSML tool instructions for Responses tools", async () => {
		const prompts = [];
		const provider = {
			async generateText(input) {
				prompts.push(input.prompt);
	...
	test("adds DSML tool instructions for wrapped Responses tools", async () => {
		const prompts = [];
		const provider = {
			async generateText(input) {
				prompts.push(input.prompt);
	...
	test("omits tool instructions for plain Responses requests", async () => {
		const prompts = [];
		const provider = {
			async generateText(input) {
				prompts.push(input.prompt);
	...
	test("passes OpenAI referenced file ids from chat request fields to provider", async () => {
	...
	test("does not treat nested inline input_file file.id as an existing file ref", async () => {
		let seenFiles = null;
		let seenFileRefs = null;
		const resp = await handleChat(
			{
	...
	test("passes top-level OpenAI attachments inline uploads to provider", async () => {
		let seenFiles = null;
		let seenFileRefs = null;
		const resp = await handleChat(
			{
	...
	test("canonicalizes non-stream structured OpenAI chat JSON output", async () => {
		const resp = await handleChat(
			{
				model: "gemini-3.5-flash",
				messages: [{ role: "user", content: "return json" }],
	...
	test("formats OpenAI error envelopes usage chunks and response output edges", async () => {
	...
	test("rejects missing OpenAI Responses request objects", async () => {
		const resp = await handleResponses(undefined, baseConfig(), fakeProvider());
		assert.equal(resp.status, 400);
		const body = await resp.json();
