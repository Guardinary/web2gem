	...
	async writeRefreshedCookie(id, update) {
		...
		if (this.writeResult.changed) {
			...
			row.cookie_hash = await sha256Hex(
				normalizeGeminiCookieHeader(update.cookieHeader),
			);
...
describe("gemini account runtime", () => {
	test("leases the least-used selectable account and derives runtime auth from its cookie", async () => {
	...
	test("deduplicates refreshes and updates the active lease config after rotation", async () => {
		const store = new FakeStore([account("a")]);
		let rotateCalls = 0;
		const pool = new AccountPoolService(store, {
			nowMs: () => 120000,
	...
	test("requires fresh bootstrap and applies structured admin status after rotation", async () => {
		const missingStore = new FakeStore([account("missing-at")]);
		const missingPool = new AccountPoolService(missingStore, {
			nowMs: () => 120000,
			rotateCookie: async () =>
				new Response(null, {
					status: 200,
		...
		const restrictedPool = new AccountPoolService(restrictedStore, {
			nowMs: () => 120000,
			rotateCookie: async () =>
				new Response(null, {
					status: 200,
	...
	test("prefers fresh known-capable accounts and keeps unknown fallback configurable", async () => {
		const nowMs = 100000;
		const rows = [
		...
		const fallbackPool = new AccountPoolService(fallbackStore, {
			nowMs: () => nowMs,
			rotateCookie: async () => new Response(null, { status: 200 }),
		});
	...
	test("uses Basic for stale known-family fallback and forbids dynamic fallback", async () => {
		const nowMs = 100000;
		const staleRoute = {
		...
		assert.deepEqual(knownLease.selectedRoute, basicRouteForFamily("pro"));
		knownLease.release();
	...
	test("builds one catalog and honors saved exact-route priority", async () => {
		const nowMs = 100000;
		const rows = [
		...
		const resolvedPro = resolveModel("gemini-3.1-pro", "gemini-3.5-flash");
		...
		rediscoveredStore.listModelRoutePriorities = store.listModelRoutePriorities;
		const rediscoveredPool = new AccountPoolService(rediscoveredStore, {
			nowMs: () => nowMs,
			rotateCookie: async () => new Response(null, { status: 200 }),
		});
		assert.deepEqual(
		...
		const stalePool = new AccountPoolService(staleStore, {
			nowMs: () => nowMs,
			rotateCookie: async () => new Response(null, { status: 200 }),
		});
		assert.deepEqual(
	...
	test("writes observed response cookies only after filtering and identity checks", async () => {
