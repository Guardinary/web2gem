...
export function classifyGeminiAccountOutcome(error: unknown, nowMs: number): GeminiAccountOutcome {
  const upstreamStatus = numericField(error, "upstreamStatus") ?? numericField(error, "status");
  const code = stringField(error, "code");
  const text = safeErrorText(error);
  const lower = text.toLowerCase();

  if (AUTH_STATUSES.has(Number(upstreamStatus)) || code === "invalid_gemini_cookie" || hasMarker(lower, AUTH_MARKERS)) {
    return {
      kind: "failure",
      failureKind: "auth",
  ...
  if (RATE_LIMIT_STATUSES.has(Number(upstreamStatus)) || /\b(429|quota|rate limit|usage limit|1037)\b/i.test(text)) {
    return {
      kind: "failure",
  ...
  if (hasMarker(lower, USER_ACTION_MARKERS)) {
    return durableFailure("needs_user_action", "needs_user_action", "Gemini account needs user action", upstreamStatus, code, nowMs);
  }

  if (hasMarker(lower, LOCATION_BLOCK_MARKERS) || /\b1060\b/.test(text)) {
    return durableFailure("location_or_ip_block", "hard_blocked", "Gemini account is blocked by location or IP", upstreamStatus, code, nowMs);
  }

  if (/\b(1050|1052|model invalid|capability)\b/i.test(text)) {
    return durableFailure("model_capability", "capability_mismatch", "Gemini account lacks requested capability", upstreamStatus, code, nowMs);
  }

  if (Number(upstreamStatus) >= 500 || /\b(5\d\d|network|timeout|empty response|1013)\b/i.test(text)) {
    return {
      kind: "failure",
...
function safeErrorText(error: unknown): string {
  ...
  if (typeof error === "object") {
    ...
    return [record.code, record.reason, record.message, record.status, record.upstreamStatus]
